...
export class jwtService {
  ...
  static async createRefreshJWT(userId: string, deviceId: string) {
  ...
  static async getExpirationDateFromRefreshToken(refreshToken: string) {
    const decodedToken: any = jwt.decode(refreshToken);
    const exp = new Date(decodedToken.exp * 1000).toISOString();
    return exp;
  }

  static async getIssuedAtFromJWTAccessToken(accessToken: string) {
  ...
  static async validateRefreshToken(token: string) {
    try {
      ...
      const iat = new Date(decodedToken.iat * 1000).toISOString();
      const exp = new Date(decodedToken.exp * 1000).toISOString();
      // const iat = new Date(decodedToken.iat)
